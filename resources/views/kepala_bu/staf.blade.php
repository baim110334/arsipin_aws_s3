...
<details class="mb-6 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden group">
    <summary class="p-6 flex justify-between items-center cursor-pointer list-none select-none">
        <div>
            <h3 class="text-base font-bold text-marica-navy flex items-center gap-2">
    ...
    <div class="p-6 border-t border-gray-50 bg-gray-50/30 max-h-80 overflow-y-auto divide-y divide-gray-100">
        ...
        <div class="py-3.5 flex justify-between items-center first:pt-0 last:pb-0">
            <div>
                <p class="font-bold text-marica-navy text-xs">{{ $k->nama_lengkap }}</p>
                <p class="text-[11px] text-gray-400 mt-0.5">{{ $k->email }} • Hub: {{ $k->no_hp ?? '-' }}</p>
            </div>
            <form action="{{ route('kepala-bu.staf.rekrut', $k->id) }}" method="POST">
                @csrf
                <button type="submit" class="px-3 py-1.5 bg-marica-navy text-white text-[11px] font-bold rounded-xl uppercase tracking-wider hover:opacity-90 transition shadow-xs">
                    + Rekrut
                </button>
            </form>
        </div>
...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h3 class="text-sm font-bold text-marica-navy uppercase tracking-wider">Staf Aktif Unit: <span class="text-blue-600">{{ strtoupper(Auth::user()->bisnis_unit) }}</span></h3>
    </div>
    <div class="text-xs font-bold bg-white text-marica-navy px-3 py-1.5 rounded-xl border border-gray-100 shadow-2xs">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-50 text-sm">
                ...
                <tr class="hover:bg-gray-50/40 transition">
                    <td class="px-6 py-4">
                        <p class="font-semibold text-marica-navy">{{ $p->nama_lengkap }}</p>
                    ...
                    <td class="px-6 py-4">
                        @if($p->role === 'pegawai-retail')
                            <span class="px-2.5 py-1 bg-red-50 text-red-700 rounded-lg text-[11px] font-bold uppercase tracking-wider">
                                Retail
                            </span>
                        @else
                            <span class="px-2.5 py-1 bg-blue-50 text-blue-700 rounded-lg text-[11px] font-bold uppercase tracking-wider">
                                Komersial
                            </span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-center">
                        <form action="{{ route('kepala-bu.staf.keluarkan', $p->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mengeluarkan staf ini?')">
                            @csrf
                            <button type="submit" class="p-2 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-xl transition-all inline-flex items-center justify-center shadow-2xs border border-transparent hover:border-red-100">
