...
@section('header', 'Retail - ' . $bu->nama_bisnis_unit)

@section('content')
<div class="space-y-6 select-none animate-fade-in">
    
    <!-- HEADER BAR + TOMBOL ACTION ADMIN -->
    <div class="border-b border-gray-100 pb-4 flex justify-between items-end">
        <div>
            ...
            <p class="text-xs text-gray-400 mt-1 font-light max-w-2xl">
                {{ $bu->deskripsi ?? 'Silakan pilih salah satu entitas korporat (PT) di bawah ini untuk mengakses repositori berkas.' }}
            </p>
        </div>

        {{-- 🛠️ TOMBOL Aksi Khusus Admin: Edit & Hapus Bisnis Unit --}}
        @if(auth()->user()->role === 'admin')
            <div class="flex items-center gap-2">
                ...
                <form action="{{ route('bisnis-unit.destroy', $bu->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Bisnis Unit ini? Seluruh data Perusahaan di bawahnya akan ikut terhapus!')">
                    @csrf
                    @method('DELETE')
    ...
    <!-- GRID PERUSAHAAN DINAMIS -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        
        {{-- 🔄 PERULANGAN CARD PERUSAHAAN --}}
@foreach($perusahaans as $pt)
    <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:border-sky-300 hover:-translate-y-1 transition-all duration-300 group flex flex-col justify-between h-56 relative overflow-hidden">
        ...
        <div>
            <div class="flex justify-between items-start">
                ...
                <div class="flex items-center gap-1">
                    ...
                    {{-- 🛠️ FITUR EDIT & HAPUS PT KHUSUS ADMIN --}}
                    @if(auth()->user()->role === 'admin')
                        <div class="flex items-center gap-0.5 ml-1 z-10">
                            <!-- Tombol Edit PT -->
                            <button onclick="bukaModalEditPt('{{ $pt->id }}', '{{ $pt->nama_pt }}')" class="text-gray-300 hover:text-amber-500 p-1 transition-colors" title="Edit Nama PT">
                                <i class="fa-solid fa-pen-to-square text-xs"></i>
                            </button>
                            <!-- Tombol Hapus PT -->
                            <form action="{{ route('perusahaan.destroy', $pt->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus perusahaan {{ $pt->nama_pt }}? Seluruh arsip dokumen di dalamnya akan ikut terhapus!')">
                                @csrf
                                @method('DELETE')
        ...
        <a href="/retail/{{ $slug_bu }}/{{ Str::slug($pt->nama_pt) }}" class="flex justify-between items-center border-t border-gray-50 pt-3 mt-4">
            <div class="flex flex-col">
                ...
                <span class="text-[11px] text-slate-500 font-medium mt-0.5 flex items-center gap-1">
                    <i class="fa-regular fa-file-pdf text-red-500"></i> Buka Repositori Dokumen
                </span>
            </div>
...
@endforeach
        ...
        {{-- ➕ CARD "TAMBAH PERUSAHAAN BARU" (KHUSUS ADMIN) --}}
        @if(auth()->user()->role === 'admin')
...
<div id="modalTambahPt" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">
        <div class="bg-slate-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
            <h3 class="text-sm font-black text-[#0c1a3c]">Tambah Entitas PT Baru</h3>
        ...
        <form action="{{ route('perusahaan.store') }}" method="POST" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nama_pt" placeholder="Contoh: PT SCK / PT SBS / PT ABC" required class="w-full text-xs p-3 border border-gray-200 rounded-xl focus:outline-none focus:border-sky-500 uppercase font-bold">
            </div>
...
<script>
    ...
    function tutupModalEditPt() {
        document.getElementById('modalEditPt').classList.add('hidden');
    }
</script>
