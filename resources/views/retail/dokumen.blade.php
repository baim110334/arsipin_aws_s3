...
<div class="space-y-6 select-none animate-fade-in">
    ...
    <!-- ACTION BAR: PENCARIAN + FILTER JENIS DOKUMEN + UPLOAD -->
    <div class="flex flex-col md:flex-row gap-3 justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto flex-1">
            ...
            <!-- Dropdown Filter Jenis Dokumen Dinamis -->
                        ...
                        <tr class="hover:bg-gray-50/30 transition-colors group baris-dokumen" data-jenis="{{ strtolower($dokumen->tipe_keuangan) }}">
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-1.5">
                                    @if(Auth::user()->role === 'admin' || (Auth::user()->role === 'pegawai-retail' && strtolower(Auth::user()->bisnis_unit) === strtolower($bisnis_unit)))
                                        ...
                                        @else
                                            ...
                                            @else
                                                <form action="{{ route('dokumen.ajukan-hapus', $dokumen->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin mengajukan permohonan hapus berkas retail ini ke Kepala BU?')">
                                                    @csrf
...
<script>
    ...
    // 🔥 FITUR FILTER BERSAMAA: PENCARIAN & FILTER DROPDOWN JENIS DOKUMEN
