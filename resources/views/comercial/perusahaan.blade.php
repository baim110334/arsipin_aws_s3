...
@section('header', 'Komersial - ' . $nama_bu)
...
<div class="space-y-6 select-none animate-fade-in">
    ...
    <div class="border-b border-gray-100 pb-4">
        ...
        <p class="text-xs text-gray-400 mt-1 font-light">Silakan pilih entitas komersial (PT) di bawah ini untuk mengakses, meninjau, serta memvalidasi rekapitulasi data dan berkas PDF penutupan buku keuangan.</p>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($list_pt as $pt)
            <a href="/comercial/{{ $bisnis_unit }}/{{ Str::slug($pt) }}" 
        ...
        @empty
            <div class="col-span-full bg-white p-12 rounded-3xl border border-dashed border-gray-200 text-center text-gray-400">
                <div class="flex flex-col items-center justify-center gap-3">
                    ...
                    <span class="text-xs font-light">Entitas perusahaan komersial di bawah unit ini tidak ditemukan atau belum diregistrasikan.</span>
                </div>
            </div>
        @endforelse
    </div>
