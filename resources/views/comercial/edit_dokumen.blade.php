...
@section('title', 'Ubah Dokumen Komersial ' . $dokumen->nama_dokumen)
@section('header', 'Edit Informasi Dokumen Komersial')
...
@php
    ...
    // 🔥 AUTOMATIC FALLBACK PERUSAHAAN KOMERSIAL
    $namaPerusahaanFix = 'CPT';
    if (isset($perusahaan) && $perusahaan != '') {
        ...
        $namaPerusahaanFix = request()->segment(3) ?? 'CPT';
    }
@endphp

<div class="max-w-4xl mx-auto select-none animate-fade-in">
    <div class="mb-6">
        <a href="/comercial/{{ $dokumen->bisnis_unit }}/{{ Str::slug($namaPerusahaanFix) }}" class="inline-flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-[#0c1a3c] transition mb-2 uppercase tracking-wider">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-8 border-b border-gray-50 bg-gray-50/30">
            <h3 class="text-xl font-bold text-[#0c1a3c]">Form Perbarui Berkas PDF (Komersial)</h3>
            <p class="text-xs text-gray-400 mt-1 uppercase tracking-wider font-semibold">Mengubah arsip data komersial untuk <span class="text-blue-600 font-bold">{{ strtoupper(str_replace('-', ' ', $namaPerusahaanFix)) }}</span></p>
        </div>
        ...
        <form action="{{ route('dokumen.update', $dokumen->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
            ...
            <div class="flex items-center justify-end gap-4 border-t border-gray-50 pt-6">
                <a href="/comercial/{{ $dokumen->bisnis_unit }}/{{ Str::slug($namaPerusahaanFix) }}" class="px-5 py-2.5 text-xs font-bold text-gray-400 hover:text-[#0c1a3c] transition-colors uppercase tracking-wider">Batal</a>
                <button type="submit"
