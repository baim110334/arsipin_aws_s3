...
@section('title', 'Dashboard Divisi Komersial')
@section('header', 'Dashboard Divisi Komersial')

@section('content')
<div class="space-y-8 select-none animate-fade-in">
    
    <div class="p-6 bg-blue-50 border border-blue-100 rounded-3xl shadow-sm">
        <h3 class="text-lg font-black text-[#0c1a3c] tracking-tight">Selamat Datang di Panel Komersial, {{ Auth::user()->nama_lengkap ?? 'Pegawai' }}!</h3>
        <p class="text-xs text-slate-500 mt-1 font-light">
            Anda masuk dengan hak akses Pegawai Otoritas Komersial. Saat ini Anda ditempatkan pada Otoritas Wilayah kerja: <span class="font-bold uppercase text-sky-500 tracking-wide">{{ str_replace('-', ' ', Auth::user()->bisnis_unit) }}</span>
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div>
                <div class="p-6 border-b border-gray-50 bg-gray-50/20">
                    <h3 class="text-sm font-black text-[#0c1a3c] uppercase tracking-wider flex items-center gap-2">
                        <i class="fa-solid fa-folder-open text-sky-500"></i> Aktivitas Dokumen Komersial Terakhir
                    </h3>
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-gray-400 italic font-light">
                                        <div class="flex flex-col items-center justify-center gap-2">
                                            ...
                                            <span>Belum ada data aktivitas penyimpanan dokumen komersial terbaru.</span>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:border-sky-300 transition-all duration-300 flex flex-col justify-between space-y-6">
            ...
            <div class="pt-2">
                <a href="/comercial/{{ strtolower(Auth::user()->bisnis_unit) }}" class="w-full py-3 bg-[#0c1a3c] text-white text-xs font-black rounded-xl uppercase tracking-wider hover:bg-[#112554] active:scale-[0.98] transition-all duration-200 flex items-center justify-center gap-2 shadow-md shadow-blue-950/10 cursor-pointer text-center">
